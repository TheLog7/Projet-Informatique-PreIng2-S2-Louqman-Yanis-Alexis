<?php
    ...
    if(isset($_POST['send']) && $_POST['send']==true){
        ...
        if(!isset($sender['active_conv'])){
            $sender['active_conv'] = array(array($receiver['mail']=>$receiver['name']));
            $_SESSION['active_conv'] = $sender['active_conv'];
            if(!isset($receiver['active_conv'])){
                $receiver['active_conv'] = array(array($sender['mail']=>$sender['name']));
            }
            elseif (!in_array(array($sender['mail']=>$sender['name']), $receiver['active_conv'])){
                $new_talk = count($receiver['active_conv']);
                $receiver['active_conv'][$new_talk] = array($sender['mail']=>$sender['name']);
            }
        }
        elseif (!in_array(array($receiver['mail']=>$receiver['name']), $sender['active_conv'], true)){
            $new_talk = count($sender['active_conv']);
            $sender['active_conv'][$new_talk] = array($receiver['mail']=>$receiver['name']);
            $_SESSION['active_conv'] = $sender['active_conv'];
            if(!isset($receiver['active_conv'])){
                $receiver['active_conv'] = array(array($sender['mail']=>$sender['name']));
            }
